...
// @ts-ignore
import { ApiClient, updateStatus } from "@gr4vy-ct/common"
...
const updateOrder = async ({
  ...
  const replaceWith = ['orderState:' + orderUpdate.orderState, 'paymentState:' + orderUpdate.paymentState]
  let updateOrderMutationQuery = updateOrderMutation
  findFor.forEach( (tag, i) => updateOrderMutationQuery = updateOrderMutationQuery.replace(new RegExp(tag, "g"), replaceWith[i]) )
